...
- `bash tools/test-yinxing-guard.sh all` 退出码为 0，耗时 203.17 秒；包含宿主 Shell、递归 KernelSU BusyBox Shell 与确定性模块打包检查。
- `:app:testDebugUnitTest :app:assembleDebug --rerun-tasks --no-daemon` 构建成功，48 个任务重新执行，耗时 83.04 秒。43 个 JUnit XML 合计 353 tests、0 failures、0 errors、0 skipped。
...
d1023656ec1deb5a70511e03cda4075b8e06c8011d7e2cc76aa3d397be558c78  yinxing-guard-1.10.0-root-preview.15.zip
```
